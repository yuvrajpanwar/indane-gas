...
class Query extends connection{
    ...
    public function get_inserted_id($select = '') {
        global $c_link;
        $this->inserted = mysqli_insert_id($c_link);
        ...
        foreach ($keys_and_values as $key => $value) {
            ...
            elseif (is_array($value)) {
                foreach ($value as $k => $v) {
                    $insert_values[] = sprintf('%s',mysqli_real_escape_string($c_link,$v));
                }
            }
            else {
                $insert_values[] = sprintf('"%s"',mysqli_real_escape_string($c_link,$value));
            }
        }
    ...
    public function insert_multiple($table, $keys, $values, $on_duplicate_key_update = '') {
        ...
        foreach ($values as $v) {
            ...
            if (is_array($v)) {
                foreach ($v as $value) {
                    $vs[] = (!is_null($value)?sprintf('"%s"', mysqli_real_escape_string($c_link,$value)):'NULL');
                }
                $insert_values[] = '('.implode(',', $vs) . ')';
            }
            else {
                $insert_values[] = '('.mysqli_real_escape_string($v) . ')';
            }
        }
        self::_set_keys($insert_keys);
        self::_set_values($insert_values);
    ...
    public function replace($table, $keys_and_values) {
        ...
        foreach ($keys_and_values as $key => $value) {
            $replace_keys[] = $key;
            $replace_values[] = (!is_null($value)?sprintf('"%s"',mysqli_real_escape_string($c_link,$value)):'NULL');
        }
        $this->replace_into = "\n".
            'REPLACE INTO ' . $table.' (' . "\n" .
                "\t" . implode(',' . "\n\t", $replace_keys) . "\n" .
    ...
    public function get_affected() {
        ...
        return mysqli_affected_rows($c_link);
    }
                ...
                foreach ($having as $k => $v) {
                    if (is_array($v)) {
                        foreach ($v as $key => $value) {
                            $array[] = sprintf($k . ' NOT LIKE "%%%s%%"', mysqli_real_escape_string($c_link,$value));
                        }
                    }
                    else {
                        $array[] = sprintf($k . ' NOT LIKE "%%%s%%"', mysqli_real_escape_string($c_link,$v));
                    }
                }
                ...
                elseif (is_array($v)) {
                    foreach ($v as $key => $value) {
                        ...
                        elseif (is_int($k)) {
                            ...
                            $update[] = sprintf($k.' = "%s"',mysqli_real_escape_string($c_link,$value));
                        }
                    }
                }
                else {
                    $update[] = sprintf($k.' = "%s"',mysqli_real_escape_string($c_link,$v));
                }
            }
            $this->on_duplicate_key_update =
    ...
    private function _get_select() {
        ...
        if (is_array($this->select)) {
            ...
            foreach ($this->select as $k => $v) {
                if (false!==strpos($k,'%s')) {
                    $selects[] = sprintf($k,mysqli_real_escape_string($c_link,$v));
                }
                else {
                    $selects[] = $v;
                }
    ...
    private function _get_set() {
        ...
        foreach ($this->set as $k => $v) {
            ...
            elseif (is_int($v)) {
                $set_equals[] = sprintf($k . ' = %s', mysqli_real_escape_string($c_link,$v));
            }
            else {
                $set_equals[] = sprintf($k . ' = "%s"', mysqli_real_escape_string($c_link,$v));
            }
                ...
                elseif (is_array($v)) {
                    foreach ($v as $key => $value) {
                        ...
                        elseif (is_int($k)) {
                            ...
                            $where_equal_or[] = sprintf($k.' = "%s"',mysqli_real_escape_string($c_link,$value));
                        }
                    }
                }
                else {
                    $where_equal_or[] = sprintf($k.' = "%s"',mysqli_real_escape_string($c_link,$v));
                }
            }
            return
                '(' . "\n" .
    ...
    private function _get_where_equal_to() {
        ...
        if (
            ...
            foreach ($this->where_equal_to as $k => $v) {
                ...
                elseif (is_int($v)) {
                    $where_equal_to[] = sprintf($k . ' = %s',mysqli_real_escape_string($c_link,$v));
                }
                elseif (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_equal_to[] = sprintf($k . ' = "%s"', mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
    ...
    private function _get_where_greater_than() {
        ...
        if (
            ...
            foreach ($this->where_greater_than as $k => $v) {
                ...
                elseif (is_int($v)) {
                    $where_greater_than[] = sprintf($k.' > %s',mysqli_real_escape_string($c_link,$v));
                }
                else {
                    $where_greater_than[] = sprintf($k.' > "%s"',mysqli_real_escape_string($c_link,$v));
                }
    ...
    private function _get_where_greater_than_or_equal_to() {
        ...
        if (
            ...
            foreach ($this->where_greater_than_or_equal_to as $k => $v) {
                ...
                elseif (is_int($v)) {
                    $where_greater_than_or_equal_to[] = sprintf($k.' >= %s',mysqli_real_escape_string($c_link,$v));
                }
                else {
                    $where_greater_than_or_equal_to[] = sprintf($k.' >= "%s"',mysqli_real_escape_string($c_link,$v));
                }
    ...
    private function _get_where_in() {
        ...
        if (
            ...
            foreach ($this->where_in as $k => $v) {
                ...
                elseif (is_int($v)) {
                    $where_in[] = sprintf($k . ' IN(%s)', mysqli_real_escape_string($c_link,$v));
                }
                elseif (is_array($v)) {
                    $values = array();
                    foreach ($v as $value) {
                        $values[] = '"' . mysqli_real_escape_string($c_link,$value) . '"';
                    }
    ...
    private function _get_where_less_than() {
        ...
        if (
            ...
            foreach ($this->where_less_than as $k => $v) {
                ...
                elseif (is_int($v)) {
                    $where_less_than[] = sprintf($k.' < %s',mysqli_real_escape_string($c_link,$v));
                }
                else {
                    $where_less_than[] = sprintf($k.' < "%s"',mysqli_real_escape_string($c_link,$v));
                }
    ...
    private function _get_where_less_than_or_equal_to() {
        ...
        if (
            ...
            foreach ($this->where_less_than_or_equal_to as $k => $v) {
                ...
                elseif (is_int($v)) {
                    $where_less_than_or_equal_to[] = sprintf($k.' <= %s',mysqli_real_escape_string($c_link,$v));
                }
                else {
                    $where_less_than_or_equal_to[] = sprintf($k.' <= "%s"',mysqli_real_escape_string($c_link,$v));
                }
    ...
    private function _get_where_like_after() {
        ...
        if (
            ...
            foreach ($this->where_like_after as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_like_after[] = sprintf($k.' LIKE "%s%%"', mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
                    $where_like_after[] = sprintf($k.' LIKE "%s%%"', mysqli_real_escape_string($c_link,$v));
                }
            }
            return implode(' AND' . "\n\t", $where_like_after) . ' ';
        }
    ...
    private function _get_where_like_before() {
        ...
        if (
            ...
            foreach ($this->where_like_before as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_like_before[] = sprintf($k.' LIKE "%%%s"', mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
                    $where_like_before[] = sprintf($k.' LIKE "%%%s"', mysqli_real_escape_string($c_link,$v));
                }
            }
            return implode(' AND' . "\n\t", $where_like_before) . ' ';
        }
    ...
    private function _get_where_like_both() {
        ...
        if (
            ...
            foreach ($this->where_like_both as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_like_both[] = sprintf($k.' LIKE "%%%s%%"', mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
                    $where_like_both[] = sprintf($k.' LIKE "%%%s%%"', mysqli_real_escape_string($c_link,$v));
                }
            }
            return implode(' AND' . "\n\t", $where_like_both) . ' ';
        }
    ...
    private function _get_where_like_binary() {
        ...
        if (
            ...
            foreach ($this->where_like_binary as $k => $v) {
                if (!is_null($v)) {
                    $where_like_binary[] = sprintf($k.' LIKE BINARY "%s"',mysqli_real_escape_string($c_link,$v));
                }
            }
            return implode(' AND' . "\n\t", $where_like_binary) . ' ';
        }
    ...
    private function _get_where_like_or() {
        ...
        if (
            ...
            foreach ($this->where_like_or as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_like_or[] = sprintf($k.' LIKE "%%%s%%"',mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
                    $where_like_or[] = sprintf($k.' LIKE "%%%s%%"',mysqli_real_escape_string($c_link,$v));
                }
            }
            return
                '(' . "\n" .
    ...
    private function _get_where_not_equal_or() {
        ...
        if (
            ...
            foreach ($this->where_not_equal_or as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_not_equal_or[] = sprintf($k.' <> "%%%s%%"',mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
                    $where_not_equal_or[] = sprintf($k.' <> "%%%s%%"',mysqli_real_escape_string($c_link,$v));
                }
            }
            return
                '(' . "\n" .
    ...
    private function _get_where_not_equal_to() {
        ...
        if (
            ...
            foreach ($this->where_not_equal_to as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_not_equal_to[] = is_null($value) ? $key . ' IS NOT NULL' : sprintf($k . ' != "%s"', mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
                    $where_not_equal_to[] = is_null($v) ? $k . ' IS NOT NULL' : sprintf($k . ' != "%s"', mysqli_real_escape_string($c_link,$v));
                }
            }
            return
                '(' . "\n" .
                ...
                if (is_array($values)) {
                    ...
                    foreach ($values as $k => $v) {
                        ...
                        elseif (is_int($v)) {
                            ...
                            $vs[] = sprintf('"%s"', mysqli_real_escape_string($c_link,$v));
                        }
                    }
                    $where_not_in[] =
                        $key . ' NOT IN (' . "\n\t\t" .
    ...
    private function _get_where_not_like() {
        ...
        if (
            ...
            foreach ($this->where_not_like as $k => $v) {
                if (is_array($v)) {
                    foreach ($v as $key => $value) {
                        $where_not_like[] = sprintf($k.' NOT LIKE "%%%s%%"',mysqli_real_escape_string($c_link,$value));
                    }
                }
                else {
                    $where_not_like[] = sprintf($k.' NOT LIKE "%%%s%%"',mysqli_real_escape_string($c_link,$v));
                }
            }
            return implode(' AND' . "\n\t", $where_not_like) . ' ';
        }
    ...
    private function _key_value($key, $value, $operator = '=') {
        ...
        return sprintf($key . $operator . ' %s ', mysqli_real_escape_string($c_link,$value));
    }
    ...
    private function _run_query($query) {
        global $c_link;
        $this->result = mysqli_query($c_link,$query);
        
        if (!$this->result) {
            $this->mysqli_error = mysqli_error($c_link);
            
            if ($this->debug) {
                $this->error = 'Error in query: ' . $this->mysqli_error;
            }
            else {
    ...
    public  function ge_fetch_array($sql)
		{
		  global $c_link;
			$res=mysqli_query($c_link,$sql) or die("query is wrong");
			$table = array();
			if(mysqli_num_rows($res)>0)
			{
				
						$i = 0;
        				while($table[$i] = mysqli_fetch_array($res)) 
            				$i++;
        				unset($table[$i]);                                                                                  
		...
		public  function ge_fetch_assoc($sql)
		{
		  global $c_link;
			...
			if(mysqli_num_rows($res)>0)
			{
						...
						$i = 0;
        				while($table[$i] = mysqli_fetch_assoc($res)) 
            				$i++;
        				unset($table[$i]);                                                                                  
		...
		public  function ge_table($table)
		{
		  global $c_link;
			...
			$ts = mysqli_query($c_link, $sql);
			$cts = mysqli_num_rows($ts);
			while($ats = mysqli_fetch_array($ts))
			{
				if($ats['Key'] == "PRI")
				{
					$Primkey=$ats['Field'];
			...
			$res=mysqli_query($c_link, "Select * from `$table`");
			if(mysqli_num_rows($res)>0)
			{
				echo "<tr>";
				
					for($i=0;$i<mysqli_num_fields($res);$i++)
						echo "<th>". mysqli_field_name($res,$i)."</th>";
		...
		public  function secureGET($key)
    	{
    	   global $c_link;
        	...
        	$_GET[$key] = str_ireplace("script", "blocked", $_GET[$key]);
       		$_GET[$key] = mysqli_escape_string($c_link, $_GET[$key]);
			$_GET[$key] = mysqli_real_escape_string($c_link, $_GET[$key]);
    	...
    	public  function securePOST($key)
    	{
    	   global $c_link;
        	...
        	$_POST[$key] = mysqli_escape_string($c_link, $_POST[$key]);
		...
		public  function secureREQUEST($key)
    	{
    	   global $c_link;
        	...
        	$_REQUEST[$key] = mysqli_escape_string($c_link, $_REQUEST[$key]);
		...
		{
			global $c_link;
			$result = mysqli_query($c_link, $string);
		
			if ($result == false)
			{
				error_log("SQL error: ".mysqli_error($c_link)."\n\nOriginal query: $string\n");
				// Remove following line from production servers 
				die("SQL error: ".mysqli_error($c_link)."\b<br>\n<br>Original query: $string \n<br>\n<br>");
			}
			return $result;
		}
