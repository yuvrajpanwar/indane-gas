									...
									{	
										...
										?>
											 ...
											 <form action="" method="post">
												...
												<!--<a class="btn btn-primary" href="<?php echo common::get_component_link(array('add_order','edit_products_details'),array("id"=>$d['id'],"type"=>$type_for_edit)); ?>" class="btn btn-sm" title="Edit">Edit</a>-->
												<!--<a class="btn btn-primary" href="<?php echo common::get_component_link(array('add_order','download_pdf_formate'),array("id"=>$d['id'],"type"=>$user_type)); ?>" class="btn btn-sm" title="Edit">Pdf</a>-->
												<!--<a onclick="return confirm('Are you sure Cancel?')" href="<?php echo common::get_component_link(array('add_order','delete'),array("id"=>$d['id'],"type"=>$d['user_type'])); ?>" class="btn btn-primary"  title="Delete">Cancel</a>-->
...
$("#distributer").change(function(){
		...
		if(start_date=="" && to_date=="")
		{
			if(date_val=="General")
			{
				window.location.replace ('<?php echo common::get_component_link(array("add_order","cancel_list"),array("user_type"=>'General')); ?>');
			}
			if(date_val=="Registered")
			{
				window.location.replace('<?php echo common::get_component_link(array("add_order","cancel_list"),array("user_type"=>'Registered')); ?>');
			}
			
			/*var data='dt=' + date_val;
				$.ajax({
					url: '<?php echo common::get_component_link(array("add_order","search_cancel_order_handler")); ?>',
					type: 'POST',
					data: data,
					cache: false,
...
// Search validation.................................	
		$("#final_done").click(function()
		{
		 ...
		 $( '#table_content' ).on( 'click', '.delete', function () {
			 var result=confirm("Are you sure to Cancel this record?");
			if(result==true)
			{
				return true;
			}
